...
| --- | --- | --- |
| `cmd_test.go` | Help, version, and debug-info command output. | `fast` |
| `cmd_inspect_test.go` | Manifest inspection and project-not-found error output. | `fast` |
| `task_error_test.go` | Manifest and task parse errors from `bud up`. | `fast` |
| `cmd_custom_test.go` | Custom command execution, output, env injection, stdin, exit-code normalization, project-root cwd. | `fast` |
| `task_custom_test.go` | Custom task conditions, run/skip behavior, project-root cwd, task env behavior. | `fast` |
| `cmd_init_test.go` | Manifest template creation plus shell-hook activation after `bud init`. | `fast`, `shell` |
| `cmd_cd_test.go` | Project matching plus wrapper consumption of `cd:` finalizers. | `fast`, `shell` |
| `cmd_create_test.go` | Project creation, manifest template creation, `cd:` finalizer, activation after create. | `fast`, `shell` |
| `cmd_hook_test.go` | Prompt hook exit-code preservation, deferred init, immediate activation. | `shell` |
| `global_env_test.go` | Parent-shell env activation and cleanup when leaving a project. | `shell` |
| `env_envfile_test.go` | `.env` feature activation, cleanup, in-process availability, and change detection. | `shell` |
| `task_go_test.go` | Go runtime installation, activation, module command behavior, and Go task validation. | `docker` |
| `task_node_test.go` | Node runtime installation, activation, and npm install behavior. | `docker` |
| `task_python_test.go` | Python runtime activation and virtualenv behavior. | `docker` |
| `task_pip_test.go` | Pip install into activated Python environment. | `docker` |
| `task_pipfile_test.go` | Pipfile install into activated Python environment. | `docker` |
| `task_python_develop_test.go` | Editable Python package install, update detection, and extras handling. | `docker` |
